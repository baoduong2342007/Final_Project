...
void particular_data(Trie& data, string name) {
    ...
    if (t == 1) {
        ...
        if (data.exist(s) == true) {
            cout << RED << "=> This " << name << " already exists\n";
            cout << "=> Failed\n" << BLUE;
        }
        else {
    ...
    if (t == 2) {
        ...
        if (data.exist(s) == false) {
            cout << RED << "=> This " << name << " does not exist\n";
            cout << "=> Failed\n" << BLUE;
        }
        else {
            if (data.get_id(s)->cnt_transaction > 0) {
                cout << RED << "=> There exist transactions have this " << name << "\n";
                cout << "=> Failed\n" << BLUE;
            }
            else {
                cout << GREEN << "=> Successful\n" << BLUE;
    ...
    if (t == 3) {
        ...
        s = input_string();
        if (data.exist(s) == false) {
            cout << RED << "=> The old name does not exist\n";
            cout << "=> Failed\n" << BLUE;
        }
        else {
